...
const AlarmList = ({navigation}) => {
  ...
  const toggleAlarm = async id => {
    ...
    if (!JSON.parse(tmpArr[myIndex].alarm_is_active)) {
      console.log('알람 turn off');
      console.log('turn off의 id....', id);
      // LOCAL 삭제
      MyCrudModule.deleteAlarm(
        id * 1,
        success => {
          ...
          MyCrudModule.addAlarm(
            currentAlarm,
            success => {
              console.log('저장됩니다222222!!', currentAlarm);
              MyCrudModule.showAll(
                success => {
                  console.log(success);
                  setAlarms(success);
      ...
      MyCrudModule.deleteAlarm(
        ...
        async success => {
          ...
          try {
            ...
            console.log('재생성 성공???ㄷㄷ', finalObj);
            ...
            MyCrudModule.addAlarm(
              currentAlarm,
              success => {
                console.log('재생성 됩니다!!!!', currentAlarm);
                MyCrudModule.showAll(
                  success => {
                    console.log(success);
                    setAlarms(success);
  ...
  const getList = () => {
    MyCrudModule.showAll(
      success => {
        ...
        console.log('reload!!!!!!!!!!!!!!!!!!!!!!...', tmpOnAlarms);
      },
      fail => {
        console.log('AlarmList.js 에서.....', fail);
      },
    );
  };
  ...
  console.log('유즈글로벌!', useGlobal('listenerId'));
  let a = 0;
  // useEffect(() => {
  //   let a = 0;
  //   setInterval(() => {
  ...
  // useEffect(() => {
  //   console.log('change: ', listenerId);
  //   // setIsNoti(0);
  //   a = a + 1;
  //   // if (listenerId != -1) navigation.navigate('AlarmActivate', {listenerId});
  //   setIsNoti(a);
  // }, [listenerId]);
  useEffect(() => {
