...
const AlarmDetail = ({navigation, route}) => {
  ...
  const BooleanToBit = arr => {
    ...
    // console.log('BooleanToBit', tmpNum);
    return tmpNum + '';
  };
  ...
  const updateTime = value => {
    ...
    // console.log('실험ㅋ', day, day2);
    // console.log(day > day2); // true -> 지금이 더 미래, false -> 지금이 과거
    let newday = new Date();
    console.log('오늘날짜', day);
    ...
    // let hh = newday.getHours();
    // let mm = newday.getMinutes();
    // let ss = newday.getSeconds();
    console.log(`하하하하 ${DD}-${MM}-${YYYY} ${tmpTime}`);
    setDateForFinalObj(DD + '-' + MM + '-' + YYYY + ' ' + tmpTime);
    setModifiedAlarm(tmpAlarm);
    console.log('내가 고른 시간은?', value, day, yoil);
    // console.log(year, month, date, yoil);
    // console.log(newday);
  };
  ...
  const saveAlarm = async () => {
    ...
    const finalObj = {
      // alarm_id: modifiedAlarm.alarm_id * 1,
      title: 'ILUNA',
      message: notiId2 + '',
    ...
    if (modifiedAlarm.alarm_id * 1 == 0) {
      ...
      try {
        ...
        console.log('헤에에에엑?? 성공???ㄷㄷ', finalObj);
        ...
        MyCrudModule.addAlarm(
          modifiedAlarm,
          success => {
            console.log('저장됩니다222222!!', modifiedAlarm);
      ...
      try {
        const alarm = await ReactNativeAN.scheduleAlarm(finalObj);
        console.log('와우...........재생성 성공???ㄷㄷ', finalObj);
        ...
        MyCrudModule.addAlarm(
          modifiedAlarm,
          success => {
            console.log('재생성 됩니다!!!!', modifiedAlarm);
            navigation.navigate('AlarmList');
          },
          fail => {
            console.log(fail);
  ...
  // useEffect(() => {
  //   console.log(
  //     'useEffectuseEffectuseEffectuseEffectuseEffectuseEffectuseEffectuseEffect',
  //   );
  //   const dismissSubscription = RNAlarmEmitter.addListener(
  //     'OnNotificationDismissed',
  //     data => {
  ...
  //   return () => {
  //     console.log(
  //       'unmountunmountunmount!!!!!!!!!!!!!useEffectuseEffectuseEffect',
  //     );
  ...
  return (
    <View style={{backgroundColor: '#fff'}}>
      ...
      {/* <Details height={height}>
        <View style={styles.detailTitleContainer}>
          <Text style={styles.detailTitleText}>진동</Text>
        </View>
        <View style={styles.detailContentContainer}>
          <Switch
            value={tmpVibe}
